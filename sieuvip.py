...
2. Mở lần lượt từng app lên sảnh rồi đóng lại (W#!/data/data/com.termux/files/usr/bin/python
"""
SieuVip Roblox Engine - Root Cookie Auth & Realtime Auto Rejoin System for Android.
Quy trình chuẩn:
...
"""
    ...
    def launch(
        ...
        params_without_ticket = dict(params)
        if ticket:
        ...
        links = [deep_link, legacy_link]
        if ticket:
            # Ticket có thể không được bản Roblox/app clone hỗ trợ trong URI.
            # Cookie đã được nạp riêng, nên thử lại link chỉ có placeId.
            links.extend([
                f"roblox://experiences/start?{urllib.parse.urlencode(params_without_ticket)}",
                "roblox://" + urllib.parse.urlencode(params_without_ticket),
            ])

        # Thử có freeform trước, sau đó bỏ freeform vì nhiều ROM Android 10
        # từ chối --windowingMode/--bounds dù lệnh mở app thường vẫn hoạt động.
        option_variants = [opt, ""] if opt else [""]
        last_output = ""
        for current_opt in option_variants:
            for current_link in links:
                cmd = (
                    f"am start -W {current_opt} -a android.intent.action.VIEW "
                    f"-d {shlex.quote(current_link)} -p {shlex.quote(package)}"
                )
                ok, out = cls.run(cmd)
                last_output = out
        ...
        # Một số app clone chỉ đăng ký deep link sau khi Activity chính đã chạy.
        # Mở app, chờ ngắn rồi gửi lại link join không kèm freeform.
        cls.run(f"monkey -p {shlex.quote(package)} -c android.intent.category.LAUNCHER 1")
        time.sleep(1.5)
        for current_link in links:
            cmd = (
        ...
        logging.getLogger(APP_NAME).error("Deep link %s thất bại: %s", package, last_output)
        return False
...
"""
    ...
    def _worker_loop(self) -> None:
        ...
        # GIAI ĐOẠN 2: SAU KHI WARMUP XONG, BẮT BUỘC MỞ LẠI TỪNG APP
        # VÀ JOIN GAME ID NGAY. Không phụ thuộc vòng watchdog và không chờ API
        # lấy ticket, vì bước đó có thể treo/chậm khiến app không bao giờ mở lại.
        for idx, target in enumerate(enabled):
            ...
            if (self.config.auto_arrange or not calculated_bounds) and total_enabled > 0:
                calculated_bounds = calculate_grid_bounds(
                    idx, total_enabled, self.screen_width, self.screen_height
                )

            self.package_status[pkg] = "Joining..."

            # Nếu có cookie thì nạp session trước, nhưng không gọi mạng lấy ticket
            # ở lần mở đầu để bảo đảm lệnh mở game được thực thi ngay.
            if cookies and self.config.auto_login_cookies:
                raw_c = cookies[idx % len(cookies)]
                RootController.inject_cookies_and_session(
                    pkg, raw_c, self.package_users.get(pkg)
                )
            ...
            if launched:
                ...
                self.logger.error(
                    "Initial Join %s vào game id %s thất bại", pkg, target.link
                )
            time.sleep(max(1.0, self.config.between_apps_seconds))

        # GIAI ĐOẠN 3: WATCHDOG VÀ REJOIN KHI APP BỊ TẮT/VĂNG
...
armup sạch, không sort, không join game).
3. Rejoin lần lượt từng app vào game kèm chia lưới ô cửa sổ nổi (Grid Freeform) và kích hoạt Watchdog 24/7.
"""
