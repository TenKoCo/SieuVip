...
Tích hợp xác thực Cookie Rotation, cấp vé Auth Ticket và Auto Rejoin 24/7 qua Root.
"""
...
class RobloxCookieEngine:
    """Xử lý xác thực, Cookie Rotation và sinh Auth Ticket qua cURL."""
    ...
    @classmethod
    def extract_rotated_cookie(cls, raw_headers: str) -> Optional[str]:
        """Đọc header Set-Cookie để lấy .ROBLOSECURITY mới nếu server thực hiện rotate."""
        matches = re.findall(r"(?i)set-cookie:\s*\.ROBLOSECURITY=([^;]+);", raw_headers)
        if matches:
            return cls.clean_cookie(matches[-1])
        return None

    @classmethod
    def process_cookie_session(
        cls, raw_cookie: str
    ) -> Tuple[bool, str, Optional[str], Optional[str], bool, str]:
        """
        Quy trình xác thực tuyến tính:
        1. Tạo HTTP Session & gửi request kiểm tra danh tính
        2. Kiểm tra phản hồi (200 hợp lệ / 401 fail)
        3. Đọc Set-Cookie để bắt Cookie Rotation
        4. Lấy CSRF token và sinh Auth Ticket
        """
        current_cookie = cls.clean_cookie(raw_cookie)
        if len(current_cookie) < 50:
            return False, current_cookie, None, None, False, "Cookie quá ngắn hoặc không đúng định dạng"

        # Bước 1: Gửi request xác thực và bắt toàn bộ header
        cmd_validate = [
            "curl", "-s", "-i", "-m", "12",
            "https://users.roblox.com/v1/users/authenticated",
            "-H", f"Cookie: .ROBLOSECURITY={current_cookie}",
            "-H", f"User-Agent: {cls.USER_AGENT}",
            "-H", "Accept: application/json"
        ]
        res_validate = subprocess.run(cmd_validate, capture_output=True, text=True)
        if res_validate.returncode != 0 or not res_validate.stdout:
            return False, current_cookie, None, None, False, "Lỗi kết nối tới máy chủ Roblox"

        # Bước 2 & 3: Kiểm tra Rotation từ Set-Cookie
        rotated_cookie = cls.extract_rotated_cookie(res_validate.stdout)
        is_rotated = False
        if rotated_cookie and rotated_cookie != current_cookie:
            current_cookie = rotated_cookie
            is_rotated = True

        parts = res_validate.stdout.split("\r\n\r\n", 1)
        ...
        if "id" not in user_data or "name" not in user_data:
            err_msg = user_data.get("errors", [{}])[0].get("message", "401 Unauthorized - Cookie hết hạn")
            return False, current_cookie, None, None, is_rotated, f"Xác thực thất bại: {err_msg}"

        user_name = user_data["name"]

        # Bước 4: Lấy x-csrf-token
        cmd_csrf = [
            "curl", "-s", "-i", "-m", "10",
        ...
        if not csrf_match:
            return False, current_cookie, user_name, None, is_rotated, "Không lấy được x-csrf-token"

        csrf_token = csrf_match.group(1).strip()

        # Bước 5: Sinh Authentication Ticket
        cmd_ticket = [
            "curl", "-s", "-i", "-m", "10",
            "-X", "POST", "https://auth.roblox.com/v1/authentication-ticket/",
            "-H", f"Cookie: .ROBLOSECURITY={current_cookie}",
        ...
        if ticket:
            return True, current_cookie, user_name, ticket, is_rotated, "Xác thực và cấp Ticket thành công"
        return False, current_cookie, user_name, None, is_rotated, "Roblox từ chối cấp Auth Ticket"


class CookieFileManager:
    """Quản lý tệp lưu trữ danh sách Cookie trên thiết bị."""
...
class AndroidController:
    ...
    def apply_shared_prefs_cookie(self, package: str, cookie: str) -> None:
        """Nạp session vào SharedPreferences bằng quyền Root."""
        norm = RobloxCookieEngine.clean_cookie(cookie)
        full_token = (
            f"_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-into-your-account-and-rob-your-robox.--|_{norm}"
        )
        script = f"""
pkg="{package}"
    ...
    def _run_target(
        ...
        *,
        ...
        if self.config.auto_login_cookies and self.cookies:
            ...
            ok_auth, active_cookie, username, auth_ticket, rotated, msg = (
                RobloxCookieEngine.process_cookie_session(current_raw_cookie)
            )

            if rotated:
                self.logger.info("[%s] Phát hiện Cookie mới sau Rotation! Đang cập nhật lại tệp...", target.package)
                self.cookies[index % len(self.cookies)] = active_cookie
                CookieFileManager.save(self.cookie_path, self.cookies)

            if ok_auth:
                self.logger.info("[%s] Tài khoản: %s | Đã cấp vé Auth Ticket.", target.package, username)
                self.controller.apply_shared_prefs_cookie(target.package, active_cookie)
                ticket = auth_ticket
            else:
                self.logger.warning("[%s] Xác thực Cookie thất bại: %s", target.package, msg)

        if force_rejoin and not ticket:
...
def _check_and_login_cookies_menu(
    ...
    cookie_list = CookieFileManager.load(cookie_path)
    if not cookie_list:
        print(f"{Colors.RED}[!] File {cookie_path} đang rỗng. Hãy dán cookie vào file.{Colors.RESET}")
        return

    print(f"\n{Colors.CYAN}[*] Bắt đầu xác thực Cookie & cấp Ticket qua luồng tuyến tính...{Colors.RESET}")
    has_update = False

    for idx, target in enumerate(config.targets):
        current_raw_cookie = cookie_list[idx % len(cookie_list)]
        print(f"\n[*] Đang xử lý: {Colors.BOLD}{target.package}{Colors.RESET}")

        ok, active_cookie, username, ticket, rotated, msg = (
            RobloxCookieEngine.process_cookie_session(current_raw_cookie)
        )

        if rotated:
            print(f"{Colors.YELLOW}[!] Phát hiện Cookie mới sau Rotation! Đang cập nhật lại...{Colors.RESET}")
            cookie_list[idx % len(cookie_list)] = active_cookie
            has_update = True
        ...
        print(f"{Colors.GREEN}[✓] Tài khoản: {username} | Nhận vé Auth Ticket thành công.{Colors.RESET}")
        
        controller.force_stop(target.package)
        controller.apply_shared_prefs_cookie(target.package, active_cookie)
        time.sleep(0.5)

        spec = RobloxLaunchSpec.parse(target.link)
        launched, launch_msg = controller.start_deep_link(
            target.package, spec, freeform=config.freeform, bounds=target.bounds, ticket=ticket
        )
    ...
    if has_update:
        CookieFileManager.save(cookie_path, cookie_list)
        print(f"\n{Colors.GREEN}[+] Đã đồng bộ toàn bộ Cookie sau Rotation vào {cookie_path}{Colors.RESET}")
...
def _config_menu(config: RejoinConfig, config_path: Path) -> None:
    while True:
        ...
        print(f"3. Tự động nạp Vé Cookie khi Rejoin: {'ON' if config.auto_login_cookies else 'OFF'}")
...
def interactive_menu(
    ...
    while True:
        ...
        print(f"│ {Colors.MAGENTA}   5{Colors.RESET}  │ {Colors.GREEN}Đăng nhập Cookie (Kiểm tra -> Rotate -> Vào Game)      {Colors.RESET}│")
        ...
        try:
            ...
            if choice == "1":
                ...
                _check_and_login_cookies_menu(config, controller, cookie_path)
            elif choice == "13":
